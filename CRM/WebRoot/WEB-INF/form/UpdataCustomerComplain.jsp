...
.containfo {
  ...
  .tj:hover{background:green;font-size:35px}
	...
	<script type="text/javascript">
		...
		$.messager.progress({
			...
			complain.updataComplain(forMap,{
			   callback:function(data){
  ...
  <body> 
   private Integer complainId;
     ...
     <s:property value="#request.cInfo.sjzdxxByPleasedLevel.sjzdxxId"/>
		 ...
		 <h5 align="center" style="font-family:华义彩云;"> 客户投诉添加</h5>
			<form name="addComplainInfoForm"   action=" " method="post" target="main">
			      <table width="55%" height="48" border="1" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" bordercolordark="#aaaaaa"  bordercolorlight="#FFFFFF">
				    <input  type="hidden" id="complainId" value="${cInfo.complainId }" name="complainId"/>
				    <tr style="padding:5px;">
				      <td bgcolor="#eeeeee"> 投诉主题：</td>
				      <td><input type="text" name="complainTheme" id="complainTheme" value="${cInfo.complainTheme }"> </td>
				     
				      <td bgcolor="#eeeeee"> 投诉客户：</td>
				      <td><input onclick="customerInfos()" type="text" name="customerName" value="${cInfo.customerInfo.customerMc }" id="customerName" value="" placeholder="点击文本框添加哦"  readonly="readonly"/> 
				          <input  type="hidden" name="customerId" id="customerId"   value="${cInfo.customerInfo.customerId }"/> <!-- 隐藏上级id -->
				      </td>
				      ...
				      <tr style="padding:5px;">
					      ...
					      <td align="left">
					      <input type="text" name="complainPerson" id="complainPerson"  value="${cInfo.complainPersion }"> 
					      </td>
					      
					       <td bgcolor="#eeeeee">受 理 人：</td>
					       <td align="left">
					            <input type="text" name="userName" id="userLeadName" onclick="userParent()" placeholder="点击文本框添加哦"   value="${cInfo.complainTheme }" readonly="readonly"/> 
					            <input type="hidden" name="userId"   id="userLeadId"   value="${cInfo.user.userId }"/> <!-- 隐藏上级id -->
					       </td>
					  </tr>
					  ...
					  <tr style="padding:5px;">
					      ...
					      <td align="left" > 
					      <select name="customerSatisfaction" id="customerSatisfaction" ${cInfo.sjzdxxByPleasedLevel }>
					        <s:iterator value="#request.sjzdm" id="m">
					           <s:if test='#m.sjzdxxMc != ""' >
									<option value="${m.sjzdxxId }"
										<option value="${c.sjzdxxId }"
										<s:if test="#c.sjzdxxId == #request.cInfo.sjzdxxByComplainStatus.sjzdxxId">
								        selected="selected"
								  </s:if>>
					      ...
					      <td align="left">
					        <select name="complainStatus" id="complainStatus">
					           ...
					           <s:if test='#c.sjzdxxMc != ""' >
									<option value="${c.sjzdxxId }"
										<s:if test="#c.sjzdxxId == #request.cInfo.sjzdxxByPleasedLevel.sjzdxxId">
								        selected="selected"
								  </s:if>>
										 ${c.sjzdxxMc}
								</s:if>
					  ...
					  <tr style="padding:5px;">
						      ...
						      <textarea rows="5" cols="60" name="complainContent" id="complainContent">${cInfo.complainText }</textarea>
						      </td>
					  </tr>
					  ...
					  <tr style="padding:5px;" > 
						      ...
						      <td align="left"  colspan="3"> 
						          <textarea rows="5" cols="60" name="complainHandleWay" id="complainHandleWay">${cInfo.complainManage }</textarea>
						      </td>
					  </tr>  
					  <tr style="padding:5px;">    
						      <td bgcolor="#eeeeee"> 备注信息：</td>
						      <td align="left"  colspan="3"> 
						          <textarea rows="5" cols="60" name="complainRemarkInfo" id="complainRemarkInfo">${cInfo.complainRemarks }</textarea>
						      </td>
					  </tr>  
			      </table>
			      <p align="center">
