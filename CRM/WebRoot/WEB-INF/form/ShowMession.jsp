...
<script type="text/javascript">
	...
	var bodys = "";
	  mession.showDetail(id,{
	    callback:function(data){
		var obj = eval('('+data+')');
		bodys = "<p>任务编号: "+obj['msinfo'].missionId+"</p>"+    
		 "<p>任务主题: "+obj['msinfo'].missionSubject+"</p>"+    
		  "<p>发布人: "+obj['msinfo'].user.userName+"</p>"+    
		  "<p>任务优先级: "+obj['msinfo'].missionPrecedence+"</p>"+      
		 "<p>创建日期: "+obj['msinfo'].sDate+"</p>"+      
		 "<p>联系人: "+obj['msinfo'].contactsInfo.contactsName+"</p>"+    
		 "<p>客户: "+obj['msinfo'].customerInfo.customerMc+"</p>"+    
		 "<p>任务信息: "+obj['msinfo'].missionDescription+"</p>";   
...
<body>
 ...
 <table width="100%" height="48" border="1" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" bordercolordark="#aaaaaa"  bordercolorlight="#FFFFFF">
			  ...
			  <s:iterator value="#request.mession" id="mes">
			   <tr bgcolor="#eeeeee" style="padding:5px;">
				      <td align="center"> <input type="checkbox" value="${ mes.missionId}" name="selectedss" id="selectAll" > </td>
				      <td align="center"> ${mes.missionId }</td>
				      <td align="center"> ${mes.missionSubject }</td>
				      <td align="center"> ${mes.customerInfo.customerMc}</td>
				      <td align="center"> ${mes.contactsInfo.contactsName }</td>
				      <td align="center"> ${mes.sDate }</td>
				      <td align="center"> <a href="#" id="datils" onclick="showDetail(${mes.missionId})">详细信息</a></td>
				      <td align="center"><a href="#" onclick="window.open('${basePath}mes/upmession.action?upId=${mes.missionId}')" id="datils" >修改</a></td>
				      <td align="center"><a href="#" id="datils" onclick="deleteMession(${mes.missionId})"> 删除</a></td>
			  </tr>
			</s:iterator>
				  ...
				  <td colspan="12" align="center">
				      <s:if test="#request.page.currentPage">
				         <a href="mes/mession.action?currentPage=1">首页</a>
				         <a href="mes/mession.action?currentPage=${request.currentPage-1}">上一页</a>
				      </s:if>
				      <s:else>
				       首页
				       上一页
				       ...
				       <s:if test="#request.page.hasNextPage">
				         <a href="mes/mession.action?currentPage=${request.currentPage+1}">下一页</a>
				         <a href="mes/mession.action?currentPage=${request.page.totalPage }">尾页</a>
				      </s:if>
				      <s:else>
				       下一页
				       尾页
		 ...
		 <input type="button" value="添加" class="tj" onclick="window.open('${basePath}form/addMession.jsp');" />
	     <input type="button" class="tj" value="批量删除" onclick="deleteMissionAll()">
