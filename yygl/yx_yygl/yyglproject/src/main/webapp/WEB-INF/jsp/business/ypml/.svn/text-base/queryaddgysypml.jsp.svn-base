...
<script type="text/javascript">
 ...
 var gysypmladd = function(){
	...
	//执行添加函数
	  function(){
		var indexs=[];//定义一个数组，准备存放选中行的序号
		var rows = dataGrid_obj.datagrid('getSelections');	//获取数据列表中所有选中的行(数组),要求datagrid属性idField设置正确
		for(var i=0;i<rows.length;i++){//便利所有选中的行
			...
			indexs.push(index);//将选中行的序号设置到数组indexs中
			//alert(dataGrid_obj.datagrid('getRowIndex',rows[i]));
		}
...
var columns = [ [{
	...
	{field : 'bm',title : '流水号',width : 80},
	{field : 'mc',title : '通用名',width : 130},
	{field : 'jx',title : '剂型',width : 80},
	{field : 'gg',title : '规格',width : 100},
	{field : 'zhxs',title : '转换系数',width : 50},
	{field : 'scqymc',title : '生产企业',width : 180},
	{field : 'spmc',title : '商品名称',width : 150},
	{field : 'zbjg',title : '中标价',width : 50},
	{field : 'jyztmc',title : '交易状态',width : 60}
	]];
